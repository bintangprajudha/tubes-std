...
void showChildParent(ListPasien LP, ListDokter LD){
    ...
    while (currentPasien != nullptr) {
        cout << "Pasien:\n";
        cout << "  Nama: " << currentPasien->info.Nama << endl;
        ...
        if (!found) {
            cout << "  Dokter Terkait: Tidak ada dokter yang terhubung dengan pasien ini." << endl;
        }
...
void editRelasiGantiPasien(ListDokter &LD, ListPasien &LP, string id_dokter, string nik_pasien, string new_nik_pasien){  
    adr_dokter dokter = FindDokter(LD, id_dokter);
